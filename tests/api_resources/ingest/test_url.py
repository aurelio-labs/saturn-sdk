# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from saturn_sdk import SaturnSDK, AsyncSaturnSDK
from tests.utils import assert_matches_type
from saturn_sdk.types import ResponsePayload

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestURL:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_process(self, client: SaturnSDK) -> None:
        url = client.ingest.url.process(
            url="url",
        )
        assert_matches_type(ResponsePayload, url, path=["response"])

    @parametrize
    def test_method_process_with_all_params(self, client: SaturnSDK) -> None:
        url = client.ingest.url.process(
            url="url",
            chunk=True,
            quality="high",
        )
        assert_matches_type(ResponsePayload, url, path=["response"])

    @parametrize
    def test_raw_response_process(self, client: SaturnSDK) -> None:
        response = client.ingest.url.with_raw_response.process(
            url="url",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        url = response.parse()
        assert_matches_type(ResponsePayload, url, path=["response"])

    @parametrize
    def test_streaming_response_process(self, client: SaturnSDK) -> None:
        with client.ingest.url.with_streaming_response.process(
            url="url",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            url = response.parse()
            assert_matches_type(ResponsePayload, url, path=["response"])

        assert cast(Any, response.is_closed) is True


class TestAsyncURL:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_process(self, async_client: AsyncSaturnSDK) -> None:
        url = await async_client.ingest.url.process(
            url="url",
        )
        assert_matches_type(ResponsePayload, url, path=["response"])

    @parametrize
    async def test_method_process_with_all_params(self, async_client: AsyncSaturnSDK) -> None:
        url = await async_client.ingest.url.process(
            url="url",
            chunk=True,
            quality="high",
        )
        assert_matches_type(ResponsePayload, url, path=["response"])

    @parametrize
    async def test_raw_response_process(self, async_client: AsyncSaturnSDK) -> None:
        response = await async_client.ingest.url.with_raw_response.process(
            url="url",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        url = await response.parse()
        assert_matches_type(ResponsePayload, url, path=["response"])

    @parametrize
    async def test_streaming_response_process(self, async_client: AsyncSaturnSDK) -> None:
        async with async_client.ingest.url.with_streaming_response.process(
            url="url",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            url = await response.parse()
            assert_matches_type(ResponsePayload, url, path=["response"])

        assert cast(Any, response.is_closed) is True
